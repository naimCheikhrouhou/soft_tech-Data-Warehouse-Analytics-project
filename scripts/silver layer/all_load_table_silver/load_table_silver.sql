...
CASE WHEN RIGHT(CAST(uploaded_date AS nvarchar(MAX)),1) ='"' THEN 
    ...
    END;
    go
    CREATE OR ALTER PROCEDURE silver.load_candidates_cv_raw_linkedin AS-----------------------------------------------------     load_candidates_cv_raw_linkedin
BEGIN
    DECLARE 
        @start_time DATETIME, 
        @end_time DATETIME;  
...
BEGIN
    ...
    END;
    go
    CREATE OR ALTER PROCEDURE silver.load_projects AS------------------------------------------------------------------------------------------projects --------
BEGIN
    DECLARE 
        @start_time DATETIME, 
        @end_time DATETIME; 
